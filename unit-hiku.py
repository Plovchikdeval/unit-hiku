...
class Hiku(loader.Module, HikuAPI):
    ...
    strings = {
        ...
        "no_url": "<blockquote><emoji document_id=5019523782004441717>❌</emoji>You have not provided a link to the Photo.</blockquote>",
        "changed": "<blockquote><emoji document_id=5206607081334906820>✔️</emoji>The link to the photo has changed; the changes will take effect within a couple of minutes.</blockquote>",
        "default_set": "<blockquote><emoji document_id=5206607081334906820>✔️</emoji>Default photo set.</blockquote>"
    }
    ...
    strings_ru = {
        ...
        "no_url": "<blockquote><emoji document_id=5019523782004441717>❌</emoji>Вы не указали ссылку на Фото.</blockquote>",
        "changed": "<blockquote><emoji document_id=5206607081334906820>✔️</emoji>Ссылка на фото изменилась в течении пары минут изменения вступят в силу.</blockquote>",
        "default_set": "<blockquote><emoji document_id=5206607081334906820>✔️</emoji>Фото по умолчанию установлено.</blockquote>"
    }
    ...
    async def set_photo(self, message):
        """Your set photo to profile unit-hiku .set_photo default - set photo to default"""
        raw_args = utils.get_args_raw(message).split()
        if not raw_args:
            await utils.answer(message, self.strings["no_url"])
            return

        if raw_args[0].lower() == "default":
            await self._client.send_message(self.BOT, "/set_pcustom https://unit-hiku.top/files/profile.png")
            await utils.answer(message, self.strings["default_set"])
            return

        photo_url = raw_args[0]
        await self._client.send_message(self.BOT, f"/set_pcustom {photo_url}")
